...
def get_neighbors(point, previus_tile, tile, grid):
    x, y = point
    neighbors = []
        ...
        if new_x >= 0 and new_x < len(grid[0]) and new_y >= 0 and new_y < len(grid):
            if previus_tile is not None:
                if tile in allowed_connections[previus_tile]:
                    print('previus_tile', previus_tile)
                    print('tile', tile)
...
def a_star_search(start, goal, tiles, grid):
    ...
    while not frontier.empty():
        ...
        for tile in tiles:
            if tiles[tile]['available'] > 0:
                
                for next in get_neighbors(current, previus_tile, tile, grid):
                    new_cost = cost_so_far[current] + tiles[tile]['cost']
                    if next not in cost_so_far or new_cost < cost_so_far[next]: # se non c'è un costo o il nuovo costo è minore
                        cost_so_far[next] = new_cost
                        priority = new_cost + heuristic(goal, next)
                        frontier.put(next, priority)
                        came_from[next] = (current, tile)
                        
                        # piazza la tile nel punto corrente
                        grid[next[1]][next[0]] = tile
                        # decrementa il numero di tiles disponibili
                        # tiles[tile]['available'] -= 1
